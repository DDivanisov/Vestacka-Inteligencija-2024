...
if __name__ == '__main__':
    ...
    test_data = dataset[:int(len(dataset) * x)]
    train_data = dataset[int(len(dataset) * x):]
    ...
    classifierMLP1 = MLPClassifier(15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)
    classifierMLP2 = MLPClassifier(15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)
    ...
    classifierDT.fit(train_x, t_train_y)
    features = list(classifierDT.feature_importances_)
    least = features.index(min(features))

    new_data = []
    for row in dataset:
        new_data.append(row[:least] + row[least + 1:])

    test_data = new_data[:int(len(new_data) * x)]
    train_data = new_data[int(len(new_data) * x):]
    ...
    MinMax.fit(train_x)
    classifierMLP1.fit(Standard.transform(train_x), t_train_y)
    classifierMLP2.fit(MinMax.transform(train_x), t_train_y)

    print(f"Tocnost so StandardScaler: {classifierMLP1.score(Standard.transform(test_x), t_test_y)}")
    print(f"Tocnost so MinMaxScaler: {classifierMLP2.score(MinMax.transform(test_x), t_test_y)}")
