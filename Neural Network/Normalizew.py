...
if __name__ == '__main__':
    ...
    inputX = input()
    inputX = inputX.split(' ')
    inputX = [float(el) for el in inputX]

    classifier = MLPClassifier(hidden_layer_sizes=hiden, activation='relu', learning_rate_init=learn_rate, max_iter=20, random_state=0)
    train_data = dataset[:int(0.8 * len(dataset))]
    validation_data = dataset[int(0.8 * len(dataset)):]

    train_x = [row[:-1] for row in train_data]
    train_y = [row[-1] for row in train_data]
    validation_x = [row[:-1] for row in validation_data]
    validation_y = [row[-1] for row in validation_data]

    MinMax = MinMaxScaler(feature_range=(-1, 1))
    MinMax.fit(train_x)

    classifier.fit(MinMax.transform(train_x), train_y)

    accTR = classifier.score(MinMax.transform(train_x), train_y)
    accVal = classifier.score(MinMax.transform(validation_x), validation_y)

    diff = accTR - accVal
    classifier1 = MLPClassifier(hidden_layer_sizes=hiden, activation='relu', learning_rate_init=learn_rate, max_iter=20, random_state=0)
    MinMax1 = MinMaxScaler(feature_range=(-1, 1))
    train_x1 = [row[:col] + row[col + 1:] for row in train_x]
    MinMax1.fit(train_x1)
    classifier1.fit(MinMax1.transform(train_x1), train_y)
    if diff/accTR > 0.15:
        print("Se sluchuva overfitting")
        inputX = inputX[:col] + inputX[col + 1:]
        print(classifier1.predict(MinMax1.transform([inputX]))[0])
    else:
        print("Ne se sluchuva overfitting")
        print(classifier.predict(MinMax.transform([inputX]))[0])