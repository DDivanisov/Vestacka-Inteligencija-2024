...
if __name__ == '__main__':
    ...
    X = X/100
    train_data = class_zero[:int(X * len(class_zero))] + class_one[:int(X * len(class_one))]
    test_data = class_zero[int(X * len(class_zero)):] + class_one[int(X * len(class_one)):]
    ...
    classifierMLP = MLPClassifier(learning_rate_init=0.001, activation='relu', batch_size=10, random_state=0)
    classifierDT = DecisionTreeClassifier(criterion='entropy', random_state=0)
    classifieRF = RandomForestClassifier(criterion='entropy', n_estimators=4, random_state=0)
    classifierGNB = GaussianNB()

    classifierGNB.fit(train_x, train_y)
    classifieRF.fit(train_x, train_y)
    classifierDT.fit(train_x, train_y)
    classifierMLP.fit(train_x, train_y)

    c = [classifierGNB, classifieRF, classifierDT, classifierMLP]
    max = 0
    best = None
    name = ""
    for el in c:
        if el.score(test_x, test_y) > max:
            max = el.score(test_x, test_y)
            best = el
    ...
    for i in range(test_x.__len__()):
        ...
        voteO = 0
        for classifier in c:
            if classifier.predict([test_x[i]])[0] == 0:
                voteZ += 1
            else:
                voteO += 1
            if best.predict([test_x[i]])[0] == 0:
                voteZ += 2
            else:
                voteO += 2
