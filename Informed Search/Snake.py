...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        new_state = self.keep_straight(snake, green_apples, direction_snake)
        if new_state[0] is not snake:
            possibility['ProdolzhiPravo'] = new_state

        new_state = self.turn_left(snake, green_apples, direction_snake)
        if new_state[0] is not snake:
            possibility['SvrtiLevo'] = new_state

        new_state = self.turn_right(snake, green_apples, direction_snake)
        if new_state[0] is not snake:
    ...
    def h(self, node):
        return closest_apple_distance(list(node.state[0]), list(node.state[1]))

    @staticmethod
    def keep_straight(snake, green_apples, direction_snake):
        head = snake[-1]
        if direction_snake == 'down':
            if head[1] - 1 >= 0 and (head[0], head[1] - 1) not in snake:
                snake = list(snake)
            ...
            if head[1] + 1 < 10 and (head[0], head[1] + 1) not in snake:
                snake = list(snake)
                snake.append((head[0], head[1] + 1))
            ...
            if head[0] + 1 < 10 and (head[0] + 1, head[1]) not in snake:
                snake = list(snake)
            ...
            if head[0] - 1 >= 0 and (head[0] - 1, head[1]) not in snake:
                snake = list(snake)
                snake.append((head[0] - 1, head[1]))
    ...
    @staticmethod
    def turn_left(snake, green_apples, direction_snake):
        head = snake[-1]
    ...
    @staticmethod
    def turn_right(snake, green_apples, direction_snake):
        head = snake[-1]
...
def closest_apple_distance(snake_body, apples):
        snake_head = snake_body[-1]
        if apples.__len__() == 0:
            return 0
        distances = []
        for apple in apples:
            distances.append(abs(snake_head[0]-apple[0]) + abs(snake_head[1] - apple[1]))

        min_distance = min(distances)
        max_distance = max(distances)
        num_apples = len(apples)
        return min_distance + num_apples*10
...
if __name__ == "__main__":
    ...
    snake_problem = Snake((snake, tuple(zeleni_jabolki), direction_snake))
    node = astar_search(snake_problem)
    if node is not None:
