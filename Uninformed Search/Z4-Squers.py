...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        new_state = move_left(state, 1)
        if new_state != state:
            succ["Pomesti kvadratche 1 levo"] = new_state

        new_state = move_right(state, 1)
        if new_state != state:
            succ["Pomesti kvadratche 1 desno"] = new_state

        new_state = move_up(state, 1)
        if new_state != state:
            succ["Pomesti kvadratche 1 gore"] = new_state

        new_state = move_down(state, 1)
        if new_state != state:
            succ["Pomesti kvadratche 1 dolu"] = new_state

        new_state = move_left(state, 2)
        if new_state != state:
            succ["Pomesti kvadratche 2 levo"] = new_state

        new_state = move_right(state, 2)
        if new_state != state:
            succ["Pomesti kvadratche 2 desno"] = new_state

        new_state = move_up(state, 2)
        if new_state != state:
            succ["Pomesti kvadratche 2 gore"] = new_state

        new_state = move_down(state, 2)
        if new_state != state:
            succ["Pomesti kvadratche 2 dolu"] = new_state

        new_state = move_left(state, 3)
        if new_state != state:
            succ["Pomesti kvadratche 3 levo"] = new_state

        new_state = move_right(state, 3)
        if new_state != state:
            succ["Pomesti kvadratche 3 desno"] = new_state

        new_state = move_up(state, 3)
        if new_state != state:
            succ["Pomesti kvadratche 3 gore"] = new_state

        new_state = move_down(state, 3)
        if new_state != state:
            succ["Pomesti kvadratche 3 dolu"] = new_state

        new_state = move_left(state, 4)
        if new_state != state:
            succ["Pomesti kvadratche 4 levo"] = new_state

        new_state = move_right(state, 4)
        if new_state != state:
            succ["Pomesti kvadratche 4 desno"] = new_state

        new_state = move_up(state, 4)
        if new_state != state:
            succ["Pomesti kvadratche 4 gore"] = new_state

        new_state = move_down(state, 4)
        if new_state != state:
            succ["Pomesti kvadratche 4 dolu"] = new_state

        new_state = move_left(state, 5)
        if new_state != state:
            succ["Pomesti kvadratche 5 levo"] = new_state

        new_state = move_right(state, 5)
        if new_state != state:
            succ["Pomesti kvadratche 5 desno"] = new_state

        new_state = move_up(state, 5)
        if new_state != state:
            succ["Pomesti kvadratche 5 gore"] = new_state

        new_state = move_down(state, 5)
        if new_state != state:
            succ["Pomesti kvadratche 5 dolu"] = new_state
...
def move_up(old_state, who_moves):
    new_state = list(old_state)
    who = new_state[who_moves - 1]
    new_state.remove(who)
    who = (who[0], who[1] + 1)
    new_state.insert(who_moves - 1, who)
...
def move_down(old_state, who_moves):
    new_state = list(old_state)
    who = new_state[who_moves - 1]
    new_state.remove(who)
    who = (who[0], who[1] - 1)
...
def move_right(old_state, who_moves):
    new_state = list(old_state)
    who = new_state[who_moves - 1]
    new_state.remove(who)
    who = (who[0] + 1, who[1])
...
def move_left(old_state, who_moves):
    new_state = list(old_state)
    who = new_state[who_moves - 1]
    new_state.remove(who)
    who = (who[0] - 1, who[1])
...
if __name__ == '__main__':
    ...
    problem = breadth_first_graph_search(squares)
