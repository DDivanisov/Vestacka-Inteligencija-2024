...
class ArangingPilars(Problem):
    ...
    def successor(self, state):
        succ = {}
        n = list(state).__len__()
        for i in range(n):
            for j in range(n):
                if i != j:
                    new_state = move_from_n_to_m(state, i, j)
                    if new_state != state:
                        succ["MOVE TOP BLOCK FROM PILLAR " + i + " TO PILLAR " + j] = new_state
        return succ
...
def move_from_n_to_m(old_state, n, m):
    new_state = list(old_state)
    new_state = tuple(
        tuple(element if i != n else element[:-1] for i, element in enumerate(old_state))
    )
    removed_element = list(old_state[n]).pop()
    list = []
    for i in range(list(old_state).__len__()):
...
if __name__ == "__main__":
    ...
    for t in tup:
        if t == '':
            ...
            t = t.split(",")
            integer_list = [int(x) for x in t]
            state1.append(tuple(integer_list))
    state = tuple(state1)

    problem = ArangingPilars(state1)
    result = breadth_first_graph_search(problem)

    if problem is not None:
        print(result.solution())
        print(result.solution().__len__())