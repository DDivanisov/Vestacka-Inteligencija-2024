...
if __name__ == '__main__':
    ...
    col_index = int(input())
    num_estimators = int(input())
    criterion = input()
    new_record = list(map(float, input().split()))

    train_data = dataset[:int(0.85 * len(dataset))]
    train_X = [row[:-1] for row in train_data]  # Извлечете ги сите колони освен последната
    ...
    classifier = RandomForestClassifier(criterion=criterion, n_estimators=num_estimators, random_state=0)
    ...
    predicted_class = classifier.predict([new_record[:col_index] + new_record[col_index+1:]])[0]
    print(f'Predicted class: {predicted_class}')


    probabilities = classifier.predict_proba([new_record[:col_index] + new_record[col_index+1:]])[0]
